---
title: "Fitting the Model to Data"
engine: knitr
webr:
  packages:
    - pracma
...
# Function Inputs: A time, proportions at that time, and model parameters
...
# Number of S,I,R at the beginning of the epidemic (initial variables)
...
::: {.callout-note title="We know the answer here"}
...
# Function Outputs: Plot of the log-likelihoods and the estimate for beta
mle <- function(plausible_beta, data) {
  ...
  # Plotting a dotted line where the estimate for beta is
...
# Function Outputs: The negative log-likelihood of the entire data set
...
# NOTE: Make the initial guess plausibly close to the true parameter
# If our guess is too far off then mle2 might return some errors
start <- list(b = 2)

# Using the L-BFGS-B algorithm / method for MLE
# Setting the lower bound to be small but positive (beta > 0)
result <- mle2(
...
1. Like we did when we coded MLE from scratch, try changing the value of `beta` in `model_parameters` in **Step 1** (line 24) and see if the maximum likelihood estimate for $\beta$ returned by `mle2` in **Step 3** reflects these changes.
:::


::: {.callout-tip title="Parameter Estimation Summary"}
