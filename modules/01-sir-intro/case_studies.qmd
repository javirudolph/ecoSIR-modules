...
Read through [@sec-the-classic-sir-epidemic-model] and [@sec-formulating-foi] if you need a refresher on how we derive these model equations.
...
Plotting all three SIR curves on a single plot will not be very helpful to us since the number of susceptibles is in the hundreds of thousands while the number of infected is only in the thousands. A base R plot will be too zoomed out to show any useful details. To produce a useful plot with all three curves, a special plotting package must be used. [Step 5](#step-5) has a plot of all three curves that can be panned and zoomed into. In this section, we will just plot each curve separately using base R, starting with the susceptible curve:
...
::: {.callout-note title="Models as approximations"}
