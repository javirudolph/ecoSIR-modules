...
MLE works by maximizing the *likelihood function*. The likelihood function provides the probability of some data point ($D_i$) occurring under different values of $\beta$. We will denote the likelihood for a data point as $\mathcal{L}(\beta,D_i)$. The likelihood function takes in a value of $\beta$ and outputs the probability of observing the data point $D_i$. 
...
We will call $\mathcal{L}(\beta,D_i)$ the point likelihood function (because it gives the likelihood of individual data points) and $\mathcal{L}(\beta,S)$ the sample likelihood function (because it gives the likelihood of the entire sample of data). What these likelihood functions look like depend on the type of disease data we are using. Disease data often comes in two forms:
...
It is generally more straightforward to estimate $\beta$ when we have prevalence data, so we will stick to that for this introductory module. Let's go over the MLE algorithm using prevalence data step-by-step.

* **Step 1:** Make an assumption about the distribution of the observed data. Oftentimes, this means assuming that every piece of data was randomly drawn from a Poisson distribution whose mean $\lambda$ is equal to the predicted value for the number of infectious individuals given by the SIR model at a time $t$. 

:::{.callout-note collapse="true"}
## What is the Poisson distribution? 

In this context, the Poisson distribution describes the probability of having observed a particular number of infected individuals at a time $t$ given that the the average number of infectious individuals $\lambda$ is known. It's important to note that we are not saying the prediction from the SIR model is random. What we are really saying is that we assume each data point in the sample was drawn randomly from a Poisson distribution centered around the SIR model's prediction for what that data point should be.  

The Wikipedia article on the Poisson goes more into the specifics of the distribution itself [@wiki_poisson].  
:::

:::{.callout-note collapse="true"}
## Do we have to use the Poisson distribution?

No! The likelihood function can correspond to many different distributions; it all depends on the data. It is common to assume a Normal or Negative Binomial distribution for the observed data when constructing the likelihood. Choosing a distribution can be difficult and requires assessing which one best describes the data. For the purposes of this module, however, we will always assume that the observed data are drawn from a Poisson distribution because it makes for a relatively simple likelihood function.
:::

* **Step 2:** Set a range of plausible values for $\beta$. If we notice our prevalence data grows at a rapid rate, then we would want our range of $\beta$s to include values that are large (relative to our recovery rate). Likewise, if we notice our data grows at a very slow rate, we would set our range of $\beta$s to include values that are close to (but slightly larger than) our recovery rate. 

* **Step 3:** For each plausible value of $\beta$, iterate through every data point in the set and calculate its likelihood REWORD: PROBABILITY OF DATA . . . ($\mathcal{L}(\beta,D_i)$). Multiply the likelihoods of all the data points together and store that value. This is the sample likelihood ($\mathcal{L}(\beta,S)$).
...
The data we will be utilizing for this example is publicly avaliable through the Ottar Bjørnstad Lab at Penn State University [@bjornstad_measles_data]. The data set contains prevalence data and annual number of births for 60 British cities from 1944 to 1966 (inclusive) as well as the median population size of each city between 1944 and 1966 (inclusive). 

Our example will only utilize the prevalence data from 1944 to 1945 (52 weeks) and the median population size between 1944 and 1966 for the cities of Blackburn, Exeter, and Leeds, as seen in the table below:
...
# Importing our prevalence data for Blackburn
