...
Notice that these equations mirror exactly the compartmental flowcharts introduced in the [Section @sec-introducing-compartmental-models]. The epidemic SIR model can be representing using the following flowchart:
...
However, there are standardized procedures to obtain $\beta$ using the blackbox approach, of which one common method known as *maximum likelihood estimation* is covered in [Section @sec-formulating-transmission-rate].
...
### Formulating the Transmission Rate Using MLE {#sec-formulating-transmission-rate}
...
This whole process can be really hard to conceptualize at first without concrete examples. We go over a few code examples of MLE coded from stratch and MLE using the `bblme` package in R in [Sections @sec-coding-scratch] and [@sec-bblme]. 
...
$$
\begin{aligned}
\frac{dS}{dt} &= -\beta \frac{I}{N} S \\[10pt]
\frac{dI}{dt} &= \beta \frac{I}{N} S - \gamma I \\[10pt]
\frac{dR}{dt} &= \gamma I
\end{aligned}
$$
...
\begin{aligned}
\frac{dS}{dt} &= -\beta \frac{I}{N} S \\[10pt]
\frac{dI}{dt} &= \beta \frac{I}{N} S - 0.7 I \\[10pt]
\frac{dR}{dt} &= 0.7 I
\end{aligned}
$$
Now that we have our basic model equations, we need prevalence count data to conduct MLE to find $\beta$. We will be using the following data set for this example:

```{r}
data <- read.csv("../data/mle_example.csv")
data
```

:::{.callout-note}
This data set was actually generated based on random Poisson samples from a $\beta=1.2$ SIR model with the same recovery rate ($\gamma=0.7$), total population size $N=1000$, initial state variables ($S=999$,$I=1$,$R=0$), and units of time $t$ (days) as our example model. Keep this in mind because we can actually see how accurate our estimate for $\beta$ found using MLE is by seeing how close it is to the $\beta$ used to generate the data. 
:::

Our first step is to code a function using the package `deSolve` to solve our given SIR model for any value of $\beta$ and output what it predicts the number of infected individuals to be at various times. This is very similar our code from [Section @sec-desolve]:
...
# Function to take in values of the model parameters and a vector of times and output the predicted prevalence count at those times
pred <- function(parameters,t){
  initial_vals <- c(S=999,I=1,R=0)
  output <- ode(func=ModelFunc,y=initial_vals,parms=parameters,times=t,rtol = 1e-4,atol = 1e-4)
  
  # Extracting only the infected values (predicted prevalences)
  I_pred <- output[, "I"]
...
Now that we have our function to output the predicted prevalence counts given by our SIR model, we need to code a function to iterate through plausible values of $\beta$ and calculate the overall log-likelihood at each using our data:

```{r}
# Function to take in a named vector of parameters (beta and gamma) and the set data and output the overall log-likelihood
log_likelihood <- function(parameters,data){
  times <- data$day
  predicted <- pred(parameters,times)
  
  # The function dpois gives Poisson probabilities
  # Note we are setting the mean of the Poisson distribution to be the predicted prevalence counts given by the SIR model
  sum(dpois(x=data$prevalence_count,lambda=predicted,log=TRUE))
}

# Driver function to iterate through plausible values of beta, calculate the log-likelihood at each, and find the maximum likelihood estimator
mle <- function(plausible_beta,data){
  range_beta <- seq(0.01,plausible_beta,by=0.01)
  ...
  plot(all_ll$beta,all_ll$ll,type="l",xlab="beta",ylab="log-likelihood",main="Log-likelihood vs Beta")
  
  # Plotting a separate line where the maximum likelihood estimate for beta is and returning the estimate
...
The code above outputs that our maximum likelihood estimator for $\beta$ is 1.2 and this aligns directly with the $\beta$ we used to generate the data! Read on to the next section to see how we can code all of this in a much easier and faster way using the `bbmle` package. 
...
#### MLE Using the bbmle Package in R {#sec-bblme}
...
`mle2` has many arguments, which can be read up on in the (R documentation for  the `mle2` function)[@docs_bbmle_mle2], however there are only two required parameters:
...
# This is the function to calculate the negative log-likelihood for a given beta
...
# Try making the initial guess one that is "believable" since if it is too far off then mle2 might return some errors
start <- list(b=2)

# Using the L-BFGS-B algorithm / method for MLE 
# Setting the lower bound to be a very small number because we know beta to be non-negative
result <- mle2(minuslogl=poisson_ll,start=start,method="L-BFGS-B",lower=c(b=0.01))
...
We will go over each method in [Sections @sec-euler-method] and [@sec-desolve] and the differences between the two.
...
### deSolve Package in R {#sec-desolve}
...
Before we can call `ode`, we must code the function to be passed into the parameter `func`. For a classic SIR model on the proportion of individuals in each compartment (see [Section @sec-the-classic-sir-epidemic-model]) we do this using the following template function:
...
HOW TO DO CITATION FOR DATA?


Mention we assume a constant population as well. 
...
We are looking to model this measles outbreak by following the proportions of susceptible, infected, and recovered individuals using an epidemic SIR model, giving the following model equations:
...
The epidemic SIR model only has two parameters, the transmission rate $\beta$ and the recovery rate $\gamma$. From past literature, it is known that the average infectious period of measles is $6$ to $7$ days [@anderson_may_1982]. Recalling the average infectious period as equal to $\frac{1}{\gamma}$, we can obtain and estimated recovery rate of $\gamma=\frac{1}{6.5}=\frac{2}{13}$. 
...
The endemic SIR model can be solved using many of the same methods as the epidemic SIR model. In [Section @sec-solving-sir-equations] on solving the epidemic SIR model, we covered two main methods of doing so: Euler's method and the deSolve package in R.
