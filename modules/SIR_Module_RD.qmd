...
\tilde{I} = 0 \quad \text{or} \quad \tilde{S} = \frac{\gamma + \mu}{\beta}
...
Notice that we actually have two possible solutions for the equilibrium, one where $\tilde{I}=0$ and another when $\tilde{S} = \frac{\gamma + \mu}{\beta}$. We can solve for the values of the other variables at these points, starting with when $\tilde{I}=0$:

$$
\begin{aligned}
\tilde{I}=0 \implies  \mu- mu \tilde{S} = 0 \implies \tilde{S}=0
& \quad && \text{Substituting \tilde{I}=0 into \frac{dS}{dt}=0} \\[10pt]
\frac{d\tilde{I}}{dt} = \beta \tilde{I} \tilde{S} - \gamma \tilde{I} - \mu \tilde{I} = 0 
& \quad && \text{Focus on the infected equation} \\[10pt]
\tilde{I}(\beta \tilde{S} - \gamma - \mu ) = 0 
& \quad && \text{Factor out } \tilde{I} \text{ to solve for equilibrium} \\[10pt]
