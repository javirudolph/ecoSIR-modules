...
$$
R_e=\frac{\beta}{\gamma}(\frac{S}{N})=R_0\tilde{S}
$$
...
:::{.callout-note}
Try changing the initial proportion of susceptible individuals $S(0)$ to be above and below the rounded threshold value ($\frac{\gamma}{\beta}$
) shown. Notice that the proportion of infected individuals never increases if $S(0)<\frac{\gamma}{\beta}$ but does increase if $S(0)>\frac{\gamma}{\beta}$. 
:::
...
p &> \frac{1}{e}(1-\frac{1}{R_0}) && \text{Rearranging for p}
\end{align*}
...
This section will be quite brief since the same methods discussed in the section on formulating the recovery rate apply to the formulation of the death rate. 
