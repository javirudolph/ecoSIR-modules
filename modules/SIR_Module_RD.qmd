...
*Maximum likelihood estimation*, often shorthanded as MLE, is a statistical technique that estimates the value of an unknown parameter by maximizing the *likelihood function*.

The likelihood function, in the context of MLE for SIR models, is a function that provides the probability of observing some real-world disease data given a particular SIR model (i.e. particular values for the model parameters) (CITATION MLE PAPER). In other words, maximum likelihood estimation is a process that estimates the parameters of the SIR model to be the ones which maximizes the probability of observing real-world data from the scenario of interest. Recalling from [Section @sec-formulating-the-recovery-rate], the recovery rate is often found based on prior literature and held as a constant equal to the reciprocal of the average duration of infection. As such, for the epidemic SIR model, we only use MLE to estimate the value of the transmission rate $\beta$. 
...
to find prop to vaccinate use efficacy rates here: https://ir.nmu.org.ua/server/api/core/bitstreams/e4229560-2fd1-4a9f-bdeb-2c96b88fa29a/content
