...
data <- mutate(data, total_sum = rowSums(select(data, -WEEK, -TOTAL.SPECIMENS, -YEAR)))
...
pred <- function(parameters, t) {
  initial_vals <- c(S = parameters["N"] - data[1, 2], I = data[1, 2], R =
                      0)
  t_start <- 0
  t_end <- max(t)
...
binomial_ll <- function(b) {
  ...
  # Returns the negative log-likelihood-sum(
  dbinom(
    x = data$prevalence_count,
    size = parameters["N"],
