...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    withMathJax(
      sliderInput(
        "prob_inf",
        label = "Probability of Infection (\\(p\\))",
        min = 0.1,
        max = 1,
        value = 0.5,
        step = 0.1
...
server <- function(input, output) {
  ...
  output$plot_sir <- renderPlotly({
    df <- solve_sir(S0=1-1e-6, I0 = 1e-6,R0=0,c=input$contact,p=input$prob_inf,g=input$gamma,len=500,dt=0.1)
